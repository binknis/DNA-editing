...
#Part 2 --- BED files
#Notes: 1. Run from inside Alldbs directory - /home/alu/binknis/Analyses/Annovar/Alldbs (see paths used in createBEDfiles section)
#		2. Downstream (for genes on positive strand) and Upstream (for genes on negative strand) may be past chromosome lengths. 
#			May cause problem when trying to fetch genomic sequence, but fine for interval operations
...
my $num_bp_upstream = 1000; #CONST. number of basepairs to fetch when fetching "upstream" coordinates for promoter regions #also number of bp downstream for region after tx end
...
if ($createBEDfiles){
	foreach my $assembly (@assembly_arr){
		...
		foreach my $genedb (@genedbs){
			...
			my $downstream_file = $beddir ."/".$genedb."_downstream".$num_bp_upstream."bp.bed";  #Note: upstream and downstream are same lengths
			...
			open (my $downstream_fh, ">".$downstream_file) or die "open $downstream_file\n"; 
			...
			while(my $l = <$genedb_fh>){
				...
				if ($cdsStart != $cdsEnd){ #avoid output of 'empty' CDSs (the format writes the same position for start and end of CDS when there is no CDS).
					...
					## commented because WRONG - because it doesn't save only 5' and 3' exons and because it doesn't handle +/- strand issue. ##Instead - 5utr and 3utr EXONS are printed later
					# if ($txStart != $cdsStart){
				...
				#set file handles according to gene orientation
				my $up_utr_fh = ($strand eq '+' ? $utr5_fh : $utr3_fh); 
				my $down_utr_fh = ($strand eq '+' ? $utr3_fh : $utr5_fh); 
				#print exons to files (all exons, coding regions of exons, 5utr exons and 3utr exons)
				for my $i (0 .. $#exonStarts){
					#all exons
					print $exon_fh join("\t", $chrom, $exonStarts[$i], $exonEnds[$i], $outname, ".", $strand), "\n";
					#5utr-exons, 3utr-exons and coding-exons
					if($cdsStart != $cdsEnd){ #cds exists
						#up exons (lower coords in genome)
						if($exonStarts[$i] < $cdsStart or $exonEnds[$i] < $cdsStart){ #exon is before (or contains region before) CDS
							my $exStart = $exonStarts[$i]; #start doesn't (by previous condition) intersect CDS, so no modification needed
							my $exEnd = ($exonEnds[$i] > $cdsStart ?  $cdsStart : $exonEnds[$i]); #get only region before CDS
							print $up_utr_fh join("\t", $chrom, $exStart, $exEnd, $outname, ".", $strand), "\n";
						}
						...
						#down exons (higher coords in genome)
						if($exonEnds[$i] > $cdsEnd){ #exon is after (or contains region after) CDS
							my $exStart = ($exonStarts[$i] >= $cdsEnd ?  $exonStarts[$i] : $cdsEnd); #get only region after CDS
							my $exEnd = $exonEnds[$i]; #end doesn't intersect CDS (by previous condition), so no modification needed
							print $down_utr_fh join("\t", $chrom, $exStart, $exEnd, $outname, ".", $strand), "\n";
						}
					}
				}
				##intron BED
				...
				##upstream BED (promoter; before tx start) and downstream BED (after tx end)
				#get region upstream and downstream of gene (by genomic coordinates - not related to gene orientation!! gene orientation will be addressed at file handles chosen)
				my $upStart = $txStart - $num_bp_upstream; 
				$upStart = 0 if $upStart < 0; 
				my $upEnd = $txStart; #'upstream' excludes the tss (txStart is 0-base)
				my $downStart = $txEnd; 
				my $downEnd = $txEnd + $num_bp_upstream; 
				
				#set correct file handle for each region (up and down on genome) in relation to gene orientation for correct upstream and downstream annotation 
				my $up_fh; my $down_fh; 
				if($strand eq '+'){ #positive strand - upstream is 'up' 
					$up_fh = $upstream_fh; 
					$down_fh = $downstream_fh; 
				}
				else{ #negative strand - reverse 'upstream' and 'downstream'; so 'up' is downstream
					$up_fh = $downstream_fh; 
					$down_fh = $upstream_fh; 
				}
				if($txStart>0){ #print up (lower coordinates) only if txStart is greater than 0, otherwise there is no up region
					print $up_fh join("\t", $chrom, $upStart, $upEnd, $outname, ".", $strand), "\n"; 
				}
				print $down_fh join("\t", $chrom, $downStart, $downEnd, $outname, ".", $strand), "\n"; 
				
				##TSS BED
				my $tssZeroBase = ($strand eq '+' ? $txStart : ($txEnd-1)); #get tss based on strand
				print $tss_fh join("\t", $chrom, $tssZeroBase, $tssZeroBase+1, $outname, ".", $strand), "\n"; #txStars is 0-based so add 1 to 'end'
			}
