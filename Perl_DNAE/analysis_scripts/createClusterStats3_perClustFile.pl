...
(my $clustFile, my $outPath, my $suffix, my $f_or_sf, my $assemblyToOrgFile) = @ARGV; 
...
$assemblyToOrgFile = "/home/alu/binknis/Analyses/Utils/Map_Clades_Orgs_Assembly.txt" unless $assemblyToOrgFile; 
my %a2o = (); 
...
while (my $l = <CLUSTS>){
	...
	my $whereS = $f[10]; 
	my $whereT = $f[11]; 
	my $taxa_f = join("\t", $org, $class, $fam); 
	my $taxa_sf = join("\t", $org, $class, $fam, $subfam); 
