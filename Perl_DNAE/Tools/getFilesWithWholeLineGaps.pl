...
#input: 1. The directory of the organisms; 2. a list of classes to search in for problematic BLAST files.  
use strict; 

(my $orgDir, my @classes) = @ARGV; 

my $classRegex = "(" . join ("|",@classes) . ")"; 
	...
	foreach my $class (@classes){
		...
		foreach my $files_fam (@fams){
			...
			foreach my $seq_name (@seq_names){
				...
				print $full_seq_name . " "; 
				my $grep = "gunzip -c " . $full_seq_name . ' | grep -c -P "^Query\s+[-]+\s*$" '; #print how many problematic lines were in each file
				system ($grep); 
			}
		}
	}
