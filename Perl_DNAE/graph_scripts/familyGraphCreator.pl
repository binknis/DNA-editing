...
(my $organism , my $class, my $arg_family, my $dontMakeGraphFile, my $labelFlag, my $nonDefaultDir) = @ARGV;
my $pvalue;
my $th;
my $family;
my $graph_dir;
...
foreach my $clusterFile (@clusterFileList)
{
 if($clusterFile =~ /clusters_(\S+)/)
 {
  ...
  $clusterFile =~ /_([^_]+)_(1e-\d+)_(\d+).txt$/;
  $family = $1;
  $pvalue = $2;
  $th = $3;
  
  #skip other families
  next if $arg_family ne $family; 
  ...
  open(my $stat_file , ">>$dir/cluster_stats_".$family.".txt") || die ("the cluster_stat file is not open");
