...
(my $organism , my $class, my $family_arg, my $dontMakeGraphFile, my $labelFlag, my $nonDefaultDir) = @ARGV;
my $pvalue;
my $th;
my $family;
my $graph_dir;
...
unlink(<$dir/cluster_stats_$family_arg*>);
...
foreach my $clusterFile (@clusterFileList)
{
 if($clusterFile =~ /clusters_(\S+)/)
 {
  ...
  $pvalue = $4;
  $th = $5;
  next if $family ne $family_arg; #RUNS FOR SPECIFIC FAMILY ONLY! 
  ...
  ($edgeCount, $editedCount) = GraphTool::clustToGraph($graph_dir, $path_clusters, $dontMakeGraphFile, $labelFlag);
  ...
  mkpath($graph_dir) unless $dontMakeGraphFile;
  ($edgeCount_control, $editedCount_control) = GraphTool::clustToGraph($graph_dir , $path_clusters_control, $dontMakeGraphFile, $labelFlag); 
  ...
  print $stat_file $family."\t".$pvalue."\t".$th."\t";
